...
var n4_NState = function() {
	this.bgColor = kha__$Color_Color_$Impl_$.Black;
...
PlayState.__name__ = true;
PlayState.__super__ = n4_NState;
PlayState.prototype = $extend(n4_NState.prototype,{
	create: function() {
		Registry.PS = this;
		this.set_bgColor(kha__$Color_Color_$Impl_$._new(-1118482));
		this.player = new sprites_Player(Math.random() * n4_NGame.width,Math.random() * n4_NGame.height);
		this.add(this.player);
		this._turret = new sprites_Turret(n4_NGame.width / 2 - 10,n4_NGame.height / 2 - 10);
		this.add(this._turret);
		this._bullets = new n4_group_NTypedGroup();
		this.add(this._bullets);
		this.emitter = new n4_effects_particles_NSquareParticleEmitter(240);
	...
	,update: function(dt) {
		...
		this._bullets.forEachActive(function(bullet) {
			if(bullet.x < 0 || bullet.x > n4_NGame.width || bullet.y < 0 || bullet.y > n4_NGame.height) {
				bullet.destroy();
			}
		});
		if((this.shotTimer -= dt) <= 0) {
			this.turretFire();
			this.shotTimer = 0.2;
		}
	}
	,turretFire: function() {
		var dx = this._turret.x + this._turret.get_width() / 2 - (this.player.x + this.player.get_width() / 2);
		var dy = this._turret.y + this._turret.get_height() / 2 - (this.player.y + this.player.get_height() / 2);
		var m = -Math.sqrt(dx * dx + dy * dy);
		this.shoot(this._turret.x + this._turret.get_width() / 2,this._turret.y + this._turret.get_height() / 2,dx * 350 / m,dy * 350 / m);
	}
	,shoot: function(x,y,dx,dy) {
		...
		if(this._turret.y > n4_NGame.height) {
...
n4_NGame.init = function(Title,Width,Height,InitialState,Framerate) {
	...
	kha_System.init({ title : Title, width : Width, height : Height},function() {
...
n4_NGame.ge_update = function() {
	...
	n4_NGame.currentState.update(n4_NGame._clock.dt);
};
...
n4_entities_NSprite.prototype = $extend(n4_NEntity.prototype,{
	...
	,makeGraphic: function(Width,Height,GraphicColor) {
		...
		if(g == null) {
			...
			n4_pooling_NGraphicPool.put(Width,Height,GraphicColor,this.graphic);
		} else {
			this.set_graphic(g);
		}
		return this;
...
n4_events_NTimerManager.prototype = {
	update: function(dt) {
		...
		while(i < this.timers.length) {
			var t = this.timers[i];
			if(t.triggerTime >= this.totalElapsed) {
				t.execute();
	...
	,setTimer: function(delay,callback) {
		this.timers.push(new n4_events_NTimer(this.totalElapsed + delay,callback));
	}
...
n4_pooling_NGraphicPool.hash = function(Width,Height,ItemColor) {
	return Width + Height + ItemColor + "";
};
n4_pooling_NGraphicPool.put = function(Width,Height,ItemColor,Graphic) {
	var h = n4_pooling_NGraphicPool.hash(Width,Height,ItemColor);
...
n4_pooling_NGraphicPool.get = function(Width,Height,ItemColor) {
	var h = n4_pooling_NGraphicPool.hash(Width,Height,ItemColor);
	var _this = n4_pooling_NGraphicPool.items;
	return __map_reserved[h] != null?_this.getReserved(h):_this.h[h];
};
...
var sprites_Bullet = function(X,Y) {
	if(Y == null) {
		Y = 0;
	}
	if(X == null) {
	...
	n4_entities_NSprite.call(this,X,Y);
	this.mass = 1.5;
	this.makeGraphic(8,8,kha__$Color_Color_$Impl_$._new(-16777216));
};
$hxClasses["sprites.Bullet"] = sprites_Bullet;
sprites_Bullet.__name__ = true;
sprites_Bullet.__super__ = n4_entities_NSprite;
sprites_Bullet.prototype = $extend(n4_entities_NSprite.prototype,{
	update: function(dt) {
		if(Math.abs(Registry.PS.player.x - this.x) < Registry.PS.player.get_width() / 1.5 && Math.abs(Registry.PS.player.y - this.y) < Registry.PS.player.get_height() / 1.5) {
			Registry.PS.player.set_x((Math.random() * 0.8 + 0.1) * n4_NGame.width);
			Registry.PS.player.set_y((Math.random() * 0.8 + 0.1) * n4_NGame.height);
			var _g = 0;
			while(_g < 15) {
				++_g;
				Registry.PS.emitter.emit(this.x,this.y,9,n4_effects_particles_NSquareParticleEmitter.velocitySpread(100,Registry.PS.player.velocity.x / 2,Registry.PS.player.velocity.y / 2),n4_util_NColorUtil.randCol(0.3,0.3,0.3),0.8);
			}
			var _g1 = 0;
			while(_g1 < 9) {
				++_g1;
				Registry.PS.emitter.emit(this.x,this.y,9,n4_effects_particles_NSquareParticleEmitter.velocitySpread(75,Registry.PS.player.velocity.x / 2,Registry.PS.player.velocity.y / 2),n4_util_NColorUtil.randCol(1,0.1,0),0.8);
			}
		}
		Registry.PS.emitter.emit(this.x,this.y,3,n4_effects_particles_NSquareParticleEmitter.velocitySpread(50),n4_util_NColorUtil.randCol(0.7,0,0),0.3);
		n4_entities_NSprite.prototype.update.call(this,dt);
	}
	,__class__: sprites_Bullet
});
...
sprites_Player.prototype = $extend(n4_entities_NSprite.prototype,{
	movement: function() {
		...
		var _left = false;
		var _right = false;
		_up = n4_NGame.keys.getKey("UP");
		_down = n4_NGame.keys.getKey("DOWN");
		_left = n4_NGame.keys.getKey("LEFT");
		_right = n4_NGame.keys.getKey("RIGHT");
		if(_up && _down) {
			_down = false;
			_up = false;
		}
...
var sprites_Turret = function(X,Y) {
	if(Y == null) {
		Y = 0;
	}
	if(X == null) {
	...
	n4_entities_NSprite.call(this,X,Y);
	this.mass = 180;
	this.makeGraphic(30,30,kha__$Color_Color_$Impl_$._new(-12303292));
};
$hxClasses["sprites.Turret"] = sprites_Turret;
sprites_Turret.__name__ = true;
sprites_Turret.__super__ = n4_entities_NSprite;
sprites_Turret.prototype = $extend(n4_entities_NSprite.prototype,{
	__class__: sprites_Turret
});
