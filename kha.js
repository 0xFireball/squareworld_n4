...
Main.__name__ = true;
Main.main = function() {
	n4_NGame.init("shapes2",0,0,PlayState,60);
};
Math.__name__ = true;
...
PlayState.bulletHitPlayer = function(b,p) {
	p.set_x((Math.random() * 0.8 + 0.1) * n4_NGame.width);
	p.set_y((Math.random() * 0.8 + 0.1) * n4_NGame.height);
	var _g = 0;
	while(_g < 15) {
		++_g;
		Registry.PS.emitter.emitSquare(b.x,b.y,9,n4_effects_particles_NParticleEmitter.velocitySpread(100,p.velocity.x / 2,p.velocity.y / 2),n4_util_NColorUtil.randCol(0.3,0.3,0.3),0.8);
	}
	var _g1 = 0;
	while(_g1 < 9) {
		++_g1;
...
PlayState.prototype = $extend(n4_NState.prototype,{
	...
	,update: function(dt) {
		...
		n4_NG.overlap(this._bullets,this.player,PlayState.bulletHitPlayer);
		if(this.player.x < 0) {
			this.player.set_x(0);
		}
		...
		var tmp = this;
		if((tmp.shotTimer -= dt) <= 0) {
			this.turretFire();
			this.shotTimer = 0.2;
		}
	}
	,turretFire: function() {
		var px = this._turret.x - this.player.x;
		var py = this._turret.y - this.player.y;
		var vx = this.player.velocity.x;
		var vy = this.player.velocity.y;
		...
		var disc = adf2 * adf2 + 4 * df * (122500 - vx * vx);
		if(disc < 0) {
			return;
		}
		disc = Math.sqrt(disc);
		...
		var c1 = (adf2 + disc) / df;
		var t1 = py / c1;
		var res1 = rot(vx - c1 * px / py,c1);
		var c2 = (adf2 - disc) / df;
		var t2 = py / c2;
		var res2 = rot(vx - c2 * px / py,c2);
		var ret = [];
		if(t1 < 0) {
			ret = res2;
		} else if(t2 < 0) {
		...
		this.shoot(this._turret.x + this._turret.get_width() / 2,this._turret.y + this._turret.get_height() / 2,ret[0],ret[1]);
	}
	,shoot: function(x,y,dx,dy) {
		...
		var recoilVelocity = bullet.get_momentum().toVector().scale(-1 / this._turret.mass);
		...
		while(_g4 < 19) {
			++_g4;
			this.emitter.emitSquare(x,y,6,n4_effects_particles_NParticleEmitter.velocitySpread(45,dx / 4,dy / 4),n4_util_NColorUtil.randCol(0.5,0.5,0.5),0.6);
		}
	}
	,__class__: PlayState
});
...
sprites_Bullet.prototype = $extend(n4_entities_NSprite.prototype,{
	update: function(dt) {
		Registry.PS.emitter.emitSquare(this.x,this.y,3,n4_effects_particles_NParticleEmitter.velocitySpread(50),n4_util_NColorUtil.randCol(0.7,0,0),0.3);
		n4_entities_NSprite.prototype.update.call(this,dt);
	}
...
sprites_Player.prototype = $extend(n4_entities_NSprite.prototype,{
	...
	,movement: function() {
		...
		var left = false;
		var right = false;
		up = n4_NG.keys.pressed(["UP","W"]);
		down = n4_NG.keys.pressed(["DOWN","S"]);
		left = n4_NG.keys.pressed(["LEFT","A"]);
		right = n4_NG.keys.pressed(["RIGHT","D"]);
